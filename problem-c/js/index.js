...
function createTaskItemElement(taskObject){
  let listItemElement = document.createElement('li');
  listItemElement.textContent = taskObject.description;
  if(taskObject.complete){
    listItemElement.classList.add('font-strike');
  }
...
function renderTaskList(task){
  ...
  list.textContent = "";
  for (let i in task) {
    list.appendChild(createTaskItemElement(task[i]));
  }
  renderInput();
}
...
function addNewTask(){
  state.taskList.push({id: state.taskList.length, 
...
let addElement = document.querySelector('#add-task');
addElement.addEventListener('click', addNewTask);
